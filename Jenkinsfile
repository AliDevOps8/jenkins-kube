pipeline {
    ...
    environment {
        IMAGE_NAME = 'alidevops8/jenkins-flask-app'
        ...
        stage('Acceptance Test')
        {
            steps {
                ...
                script {
                    ...
                    def service = sh(script: "kubectl get svc jenkins-flask-service -o jsonpath='{.status.loadBalancer.ingress[0].hostname}:{.spec.ports[0].port}'", returnStdout: true).trim()
                    echo "${service}"
